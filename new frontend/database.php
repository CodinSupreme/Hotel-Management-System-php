...
// ================= CREATE USER ==================
function createUser($email, $password, $first_name, $last_name, $contact, $gender, $id_no) {
    global $pdo;
    $username = "$first_name$last_name";
    // Hash password before saving (secure!)
    $hashed_password = password_hash($password, PASSWORD_DEFAULT);

    $sql = "INSERT INTO app_customuser (email, password, username, contact, gender, id_no, first_name, last_name, salary, role, service_id,
                is_active, is_staff, is_superuser, date_joined)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0.0, null, null, true, false, false, NOW())";

    $stmt = $pdo->prepare($sql);

    try {
        $stmt->execute([$email, $hashed_password, $username, $contact, $gender, $id_no, $first_name, $last_name]);
        return true;
